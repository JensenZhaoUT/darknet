    ...
    float hier_thresh, int dont_show, int ext_output, int save_labels, char *outfile, int letter_box, int benchmark_layers)
{
    ...
    while (1) {
        if (filename) {
            snprintf(input, sizeof(buff), "%s", filename);
            if (strlen(input) > 0)
                if (input[strlen(input) - 1] == 0x0d) input[strlen(input) - 1] = 0;
        }
        else {
        ...
        // Construct the output path with the same extension
        char *base = basename(input); // Get the base name of the file
        const char *ext = get_filename_ext(base); // Get the file extension
        char name_without_ext[256];
        strncpy(name_without_ext, base, sizeof(name_without_ext) - 1);
        name_without_ext[strlen(base) - strlen(ext) - 1] = '\0'; // Remove the extension from the base name
        char output_path[300];
        snprintf(output_path, sizeof(output_path), "./images/%s", name_without_ext); // Create the output path
